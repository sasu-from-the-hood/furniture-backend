...
function Footer() {
  ...
  const [footerSettings, setFooterSettings] = useState({
    ...
    footer: {
      footer_about: { value: 'We specialize in high-quality, stylish furniture designed to bring comfort and elegance to every space.' },
        ...
        if (siteSettings && Object.keys(siteSettings).length > 0) {
          ...
          setFooterSettings(prevSettings => ({
            ...
            general: {
              ...
              site_name: { value: siteSettings.site_name || prevSettings.general.site_name.value }
            },
            contact: {
              ...prevSettings.contact,
              contact_email: { value: siteSettings.contact_email || prevSettings.contact.contact_email.value },
              contact_phone: { value: siteSettings.contact_phone || prevSettings.contact.contact_phone.value },
            ...
            if (data.settings) {
              ...
              setFooterSettings(prevSettings => {
                ...
                if (!newSettings.footer?.footer_quick_links?.value) {
                  ...
                  newSettings.footer.footer_quick_links = prevSettings.footer?.footer_quick_links;
                }

                if (!newSettings.footer?.footer_customer_service?.value) {
                  if (!newSettings.footer) newSettings.footer = {};
                  newSettings.footer.footer_customer_service = prevSettings.footer?.footer_customer_service;
                }
  ...
  useEffect(() => {
    if (siteSettings && Object.keys(siteSettings).length > 0) {
      ...
      setFooterSettings(prevSettings => ({
        ...
        footer: {
          ...prevSettings.footer,
          footer_about: { value: prevSettings.footer?.footer_about?.value || '' },
          footer_address: { value: siteSettings.contact_address || prevSettings.footer?.footer_address?.value || '' },
          footer_copyright: { value: prevSettings.footer?.footer_copyright?.value || 'All rights reserved.' },
        ...
        contact: {
          ...
          contact_email: { value: siteSettings.contact_email || prevSettings.contact.contact_email.value },
          contact_phone: { value: siteSettings.contact_phone || prevSettings.contact.contact_phone.value },
          contact_address: { value: siteSettings.contact_address || prevSettings.contact.contact_address?.value || '' }
        },
        social: {
          ...prevSettings.social,
          social_facebook: { value: siteSettings.social_facebook || '' },
  ...
  const siteName = footerSettings.general?.site_name?.value || siteSettings?.site_name || 'European Luxury';
  const aboutText = footerSettings.footer?.footer_about?.value || '';
  const address = footerSettings.footer?.footer_address?.value || siteSettings?.contact_address || '';
  const copyright = footerSettings.footer?.footer_copyright?.value || 'All rights reserved.';
  const email = footerSettings.contact?.contact_email?.value || siteSettings?.contact_email || '';
  const phone = footerSettings.contact?.contact_phone?.value || siteSettings?.contact_phone || '';
  ...
  try {
    // Try to get the value from footerSettings
    const linksFromSettings = footerSettings.footer?.footer_customer_service?.value;
    ...
    <footer className="site-footer border border-solid">
      <div className="block w-full h-auto bg-black">
        <div className="container mx-auto p-4">
          {loading ? (
            <div className="text-center py-4 text-gray-400">Loading footer information...</div>
          ) : (
            <>
              <div className="flex flex-wrap justify-between">
                ...
                <div className="w-full sm:w-1/2 md:w-1/4 mb-6">
                  ...
                  <p className="text-gray-400">
                    {aboutText || "We specialize in high-quality, stylish furniture designed to bring comfort and elegance to every space."}
                  </p>
                </div>
                ...
                <div className="w-full sm:w-1/2 md:w-1/4 mb-6">
                  ...
                  {email && (
                    <div className="flex items-center mb-3">
                      <svg className="w-5 h-5 text-gray-300 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  ...
                  {phone && (
                    <div className="flex items-center mb-3">
                      ...
                      <a href={`tel:${phone.replace(/[^0-9+]/g, '')}`} className="text-gray-300 hover:text-white">{phone}</a>
                    </div>
                  )}
